...
def img_path_sub_sub_category(instance, filename):
    return (
        f"{instance.main_category}/{instance.sub_category}/{instance.name}/{filename}"
    )
...
class Sub_sub_Category(models.Model):
    main_category = models.ForeignKey(
        ...
        blank=True,
        null=True,
        related_name="main_category_to_sub",
        on_delete=models.PROTECT,
    )
    name = models.CharField(max_length=200)
    slug = models.CharField(max_length=200)
    ...
    def get_url2(self):
        return reverse(
            ...
            kwargs={
                ...
                "sub_sub_category_slug": self.slug,
            },
        )
...
class Produkt(models.Model):
    ean = models.ForeignKey(
        Brand, blank=True, null=True, related_name="ean", on_delete=models.PROTECT
    )
    ...
    brand = models.ForeignKey(
        Brand, blank=True, null=True, related_name="brand", on_delete=models.PROTECT
    )
    ...
    def get_url(self):
        return reverse(
            ...
            kwargs={
                "main_category_slug": self.main_category.slug,
                "sub_category_slug": self.category.slug,
                "sub_sub_category_slug": self.sub_category.slug,
