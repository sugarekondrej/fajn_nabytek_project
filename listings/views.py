...
def listings_view(
    request, main_category_slug=None, sub_category_slug=None, sub_sub_category_slug=None
):
    categories = None
    ...
    if sub_sub_category_slug != None:
        categories = models.Sub_Category.objects.get(
            slug=sub_category_slug, main_category__slug=main_category_slug
        )
        category_filter = models.Sub_sub_Category.objects.get(
            slug=sub_sub_category_slug
        )
        main_categories = get_object_or_404(
            models.Main_Category, slug=main_category_slug
        )
        sub_categories = models.Sub_sub_Category.objects.get(slug=sub_sub_category_slug)
        products = models.Produkt.objects.filter(
            category=categories,
        ...
        products = models.Produkt.objects.filter(
            category=categories, main_category=main_categories
        )
        sub_filter = models.Sub_sub_Category.objects.filter(
...
def product_detail(
    ...
    sub_category_slug,
    product_slug,
    sub_sub_category_slug=None,
):
    if sub_sub_category_slug != None:
        single_product = models.Produkt.objects.get(
            main_category__slug=main_category_slug,
            category__slug=sub_category_slug,
            sub_category__slug=sub_sub_category_slug,
            slug=product_slug,
        )
    elif sub_category_slug != None:
        single_product = models.Produkt.objects.get(
...
def categories(request, main_category_slug=None, sub_category_slug=None):
    ...
    if sub_category_slug != None:
        categories = models.Sub_Category.objects.get(
            main_category__slug=main_category_slug, slug=sub_category_slug
        )
    context = {"listings": categories}
