...
def detsky_pokoj(request):
    mistnost = Sub_Category.objects.filter(
        main_category__name__icontains="Dětský pokoj"
    )
...
def kuchynske_linky_links(request):
    links = Sub_sub_Category.objects.filter(
        sub_category__name__icontains="Kuchyňské linky"
    )
...
def jidelni_stolky_links(request):
    links = Sub_sub_Category.objects.filter(
        sub_category__name__icontains="Jidelní stolky"
    )
...
def postele(request):
    links = Sub_sub_Category.objects.filter(
        sub_category__name__icontains="Postele",
        main_category__name__icontains="Dětský pokoj",
    )
